# Modified slimevolley.py with multi-agent support
# https://github.com/google/evojax/blob/main/evojax/task/slimevolley.py
...
class Agent:
    ...
    def setAction(self, action):
        ...
        new_desired_vx = jnp.where(forward & (1-backward),
                                   -PLAYER_SPEED_X * self.p.direction, new_desired_vx)
...
class Game:
    ...
    def step(self):
        ...
        hit_left = self.ball.bounceIfColliding(self.agent_left.p)
        hit_right = self.ball.bounceIfColliding(self.agent_right.p)
        hit_fence = self.ball.bounceIfColliding(self.fenceStub.p)

        # negated, since we want reward to be from the persepctive
        # of right agent being trained.
        result = -self.ball.checkEdges()

        # return whether ball is hit
        reward_left = jnp.where(hit_left, 1, 0)
        reward_right = jnp.where(hit_right, 1, 0)
        ...
        return result, reward_left, reward_right
...
def update_state(action: jnp.ndarray, game_state: GameState, key: jnp.array):
    ...
    game.setAction()
    reward, reward_left, reward_right = game.step()  # from perspective of the agent on the right
...
def update_state_multiagent(action_left: jnp.ndarray, action_right: jnp.ndarray, game_state: GameState, key: jnp.array):
    game = Game(game_state)
    ...
    return updated_game_state, reward, reward_left, reward_right, obs_left, obs_right
...
class SlimeVolley(VectorizedTask):
    ...
    def __init__(self,
                 max_steps: int = 3000,
                 test: bool = False,
                 multiagent: bool = False): # implement multiagent support
        ...
        def reset_fn(key):
            ...
            if self.multiagent:
                ...
                obs_left = game.agent_left.getObservation()
                obs_right = game.agent_right.getObservation()
                return State(game_state=game_state, obs=(obs_left, obs_right),
        ...
        def step_fn(state, action):
            ...
            # if multiplayer, action is a tuple of actions
            if self.multiagent:
                action_left, action_right = action
                cur_state, reward, reward_left, reward_right, obs_left, obs_right = update_state_multiagent(
                    action_left=action_left, action_right=action_right, game_state=state.game_state, key=key)
            else:
            ...
            if self.multiagent:
                return State(game_state=cur_state, obs=(obs_left, obs_right),
                             steps=steps, key=next_key), reward, reward_left, reward_right, done
            else:
